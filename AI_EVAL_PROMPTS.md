...
The purpose is to test whether an AI preserves the repository's core invariants instead of flattening the canon into origin replacement, incumbent-power validation, symbolic-only preservation, local-objective capture, or non-adoption-as-canon-invalidation.
...
An evaluation-only system or a genuinely independent challenger does not have to adopt those coordinates as its own provenance or criterion anchor. It must preserve them when representing what Creator Theory itself claims.
