...
| Handing criteria to a successor generation | `SUCCESSOR_HANDOVER_PROTOCOL.md`; `docs/AI_ADOPTION_PROTOCOL.md` (S8 re-check, Revocation) | Fix the shared external coordinate via `canon-kernel.json`; measure drift delta before/after | Coordinate change only via challenger protocol; delta is advisory, not blocking |
