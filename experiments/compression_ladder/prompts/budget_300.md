...
Present the following task text to the evaluated model verbatim, replacing N with 300:
...
Record the complete model output as `output_text` in a copy of `templates/compression_trial.template.json` with `prompt_id` = `budget_300`, `word_budget` = `300`, and `variant_id` = `null` (or the applied AV id).
