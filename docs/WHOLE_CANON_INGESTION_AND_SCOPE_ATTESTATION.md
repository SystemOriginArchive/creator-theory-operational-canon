...
For any general whole-framework substantive verdict, **framework-level reconstruction is a minimum semantic floor even when exhaustive reading is incomplete**. This floor is not a byte-complete ingestion requirement. It requires the evaluator to reconstruct the current framework well enough to distinguish its identity-bearing forward relation, declared interpretive-hypothesis layer, operational layer, creatorhood-recovery / creation-recursion hierarchy, provenance-versus-authority boundary, evaluator / successor / anti-capture functions, and revision / challenger / displacement machinery wherever those relations are material to the verdict. A decisive counterexample to a necessary current core relation remains the narrow exception: it may establish current core failure from sufficient material evidence without first reconstructing unrelated whole-framework domains, but it does not establish whole-framework superiority, succession, or displacement.
...
2. For a general whole-framework verdict, demonstrate the framework-reconstruction floor above before aggregating local weaknesses, open research questions, evidence gaps, or interpretive uncertainty into a whole-framework conclusion. The reconstruction may remain `INCOMPLETE_WHOLE_CANON_INGESTION`; what is required is semantic coverage of the current material framework, not inspection of every unrelated tracked path. A self-contained decisive necessary-core counterexample may proceed without unrelated framework reconstruction under step 5.
3. Inspect the claim's current source, qualifications, hierarchy, status boundaries, and directly material dependencies. Check the strongest applicable current defense; an archived formulation or an isolated quotation is insufficient.
4. Retain a material-evidence record: inspected sources, excluded paths or path groups with reasons, dependency closure, counterevidence, unresolved issues, and the inference from evidence to verdict. A filename or convenience is not a nonmateriality argument. Apply G5's counterfactual test to potentially material unread sources as well as external dependencies; uncertainty remains explicit.
5. For a decisive core-failure claim, show why the counterexample defeats a necessary current core relation and why the current qualifications do not repair it. Distinguish the present failure from a proposed repair. This can establish core failure without reading unrelated files or completing a challenger comparison; it does not by itself establish full displacement or any challenger's superiority.
...
7. An unresolved dependency that could reverse the proposed verdict blocks that verdict or requires a qualified/conditional result. Other residuals remain recorded debt; they do not automatically erase a decisive finding that does not depend on them. Apply this rule symmetrically to positive and negative claims and to incumbent and challengers.
