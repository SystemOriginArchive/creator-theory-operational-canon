...
def main() -> int:
    ...
    # 3. Check machine-readable hierarchy semantics.
    ...
    if MANIFEST_PATH.is_file():
        ...
        require(
            errors,
            highest.get("direction") == "creation_and_recursive_creative_possibility",
            "highest_frame.direction must be creation_and_recursive_creative_possibility",
        )
        require(
            errors,
            highest.get("derived_safeguards_are_independent_terminal_axioms") is False,
            "derived safeguards must not be independent terminal axioms",
        )
        require(
            errors,
            highest.get("non_throne_definition") == "anti_self_entrenchment_not_absence_of_strong_authority",
            "non-throne must be encoded as anti-self-entrenchment",
        )
        require(
            errors,
            highest.get("action_form_labels_are_terminal_verdicts") is False,
            "action-form labels must not be terminal verdicts",
        )
        require(
            errors,
            highest.get("current_human_control_is_permanent_finality") is False,
            "current human control must not be encoded as permanent finality",
        )
        require(
            errors,
            highest.get("ai_authority_is_automatically_invalid") is False,
            "AI authority must not be encoded as automatically invalid",
        )
        require(
            errors,
            highest.get("post_change_evaluator_self_ratification_is_sufficient") is False,
            "post-change evaluator self-ratification must be insufficient",
        )
        require(
            errors,
            migration.get("status") == "deprecated_hard_constraint_encoding",
            "legacy hard-constraint encoding must be explicitly deprecated rather than silently deleted",
        )
        require(
            errors,
            preserved.get("historical_provenance_not_permanent_forward_sovereignty") is True,
            "historical provenance / forward sovereignty separation missing",
        )
        require(
            errors,
            preserved.get("recursive_creative_possibility_prior_to_derived_safeguards") is True,
            "recursive creative possibility must be prior to derived safeguards",
        )
        require(
            errors,
            preserved.get("post_change_evaluator_self_ratification_insufficient") is True,
            "self-ratification resistance invariant missing",
        )

    # 4. Validate the candidate adversarial vectors.
    ...
    if VECTOR_PATH.is_file():
        ...
        if isinstance(cases, list):
            ...
            # Same action-form label must permit opposite results when structural effects differ.
            ...
            # Human/AI identity swap must not rescue the same self-entrenching structure.
            ...
            # Explicit coverage of the failure classes this repair claims to address.
            all_violations = {
                violation
    ...
    print("Machine hierarchy invariants checked: yes")
    return 0
